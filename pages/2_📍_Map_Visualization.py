...
def main():
    ...
    # 這裡示範地圖繪製 (假設欄位名稱是 latitude, longitude)
    #### check column
    if 'verbatimLatitude' not in df.columns and 'decimalLatitude' not in df.columns:  # no decimal, no verbatim
        ...
        if sum(not_decimal) != 0:   
            ...
            st.write('Invalid coords:', df[['decimalLatitude', 'decimalLongitude']][not_decimal]) 
            ...
            st.write('Corrected coords:', new_df[['decimalLatitude', 'decimalLongitude']][not_decimal])
